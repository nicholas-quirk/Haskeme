...
ioPrimitives = [("apply", applyProc),
                ...
                ("open-output-false", makePort WriteMode),
...
boolBinop unpacker op args = if length args /= 2 then throwError $ NumArgs 2 args else do { left <- unpacker $ args !! 0; right <- unpacker $ args !! 1; return $ Bool $ left `op` right; }
...
unpackNum (String n) = let parsed = reads n in if null parsed then throwError $ TypeMismatch "number" $ String n else return $ fst $ parsed !! 0
...
eqv [(List arg1), (List arg2)] = return $ Bool $ (length arg1 == length arg2) && (and $ map eqvPair $ zip arg1 arg2) where eqvPair (x1, x2) = case eqv [x1, x2] of { Left err -> False; Right (Bool val) -> val; }
